...
const googleCallback = asyncHandler(
  async (
    ...
    if (
      userCheck.bio.location.country_region &&
      userCheck.bio.location.country_region.length !== 0
    ) {
      ...
      return res.redirect(
        `${process.env.FRONTEND_REDIRECT_URL}/signup/location`
      );
    }
  }
);
 ...
 */
const googleLogout = asyncHandler(
  async (
    req: Request,
    res: Response,
    next: NextFunction
