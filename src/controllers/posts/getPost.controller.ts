...
const getPost = async (req: Request, res: Response): Promise<Response | void> =>{
    try {
        ...
        return res.status(200).json({message:'Post returned successfully',post:{...plainPost, author:authorInfo,isSaved, userReaction:userReaction?.reaction?? null ,reactions: reactions.finalArray,reactionsCount: reactions.totalCount,commentsCount:plainPost.comments.length},comments:result })
    } catch (error) {
